...
public class BoardModel
{
    ...
    ChessType[,] _data = new ChessType[BoardManager.CrossCount, BoardManager.CrossCount];

    // 获取棋盘数据 
    public ChessType Get( int x, int y )
    {
        if (x < 0 || x >= BoardManager.CrossCount)
            return ChessType.None;

        if (y < 0 || y >= BoardManager.CrossCount)
            return ChessType.None;
...
#region 检查连接情况
    ...
    int CheckVerticalLink(int px, int py, ChessType type)
    {
        ...
        // 朝上
        for (int y = py  + 1; y < BoardManager.CrossCount; y++)
        {
            if (Get(px, y) == type)
            {
                linkCount++;
    ...
    int CheckHorizentalLink(int px, int py, ChessType type)
    {
        ...
        // 朝右+
        for (int x = px+1; x < BoardManager.CrossCount; x++)
        {
            if (Get(x, py) == type)
            {
                linkCount++;
    ...
    int CheckBiasLink(int px, int py, ChessType type)
    {
        ...
        // 右上
        for (int x = px + 1, y = py + 1; x < BoardManager.CrossCount && y < BoardManager.CrossCount; x++, y++)
        {
            if (Get(x, y) == type)
            {
                linkCount++;
        ...
        linkCount = 1;
        // 左上
        for (int x = px -1 , y = py+ 1; x >=0 && y < BoardManager.CrossCount; x--, y++)
        {
            if (Get(x, y) == type)
            {
                linkCount++;
        ...
        // 右下
        for (int x = px + 1, y = py - 1; x < BoardManager.CrossCount && y >= 0; x++, y--)
        {
            if (Get(x, y) == type)
            {
                linkCount++;
