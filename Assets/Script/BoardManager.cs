...
public class BoardManager : MonoBehaviour {
    ...
    // 存储每个交叉点按钮信息
    Dictionary<int, CrossPoint> _crossMap = new Dictionary<int, CrossPoint>();
    ...
    public void Reset( )
    {
        ...
        for (int x = 0; x < BoardManager.CrossCount; x++)
        {
            for (int y = 0; y < BoardManager.CrossCount; y++)
            {
                ...
                pos.x = -BoardManager.HalfSize + x * CrossSize;
                pos.y = -BoardManager.HalfSize + y * CrossSize;
                pos.z = 1;
                crossObj.transform.localPosition = pos;

                // 记录格子信息
                var cross = crossObj.GetComponent<CrossPoint>();
                cross.GridX = x;
                cross.GridY = y;
    ...
    public CrossPoint GetCross(int gridX, int gridY )
    {        
        CrossPoint crossPoint;
        if ( _crossMap.TryGetValue( MakeKey(gridX, gridY), out crossPoint ) )
        {
            return crossPoint;
        }
