...
class AI
{
    ...
    //玩家的可能性
    bool[,,] _ptable = new bool[BoardManager.CrossCount, BoardManager.CrossCount, MaxFiveChainCount];

    //电脑的可能性
    bool[, ,] _ctable = new bool[BoardManager.CrossCount, BoardManager.CrossCount, MaxFiveChainCount];

    //记录2位玩家所有可能的连珠数，-1则为永远无法5连珠
    int[,] _win = new int[2, MaxFiveChainCount];

    //记录每格的分值
    int[,] _cgrades = new int[BoardManager.CrossCount, BoardManager.CrossCount];
    int[,] _pgrades = new int[BoardManager.CrossCount, BoardManager.CrossCount];

    //记录棋盘
    int[,] _board = new int[BoardManager.CrossCount, BoardManager.CrossCount];
    ...
    public AI( )
    {
        for ( int i = 0;i<BoardManager.CrossCount;i++)
        {
            for ( int j = 0;j<BoardManager.CrossCount;j++)
            {
                _pgrades[i, j] = 0;
                _cgrades[i, j] = 0;
                _board[i, j] = 0;
        ...
        //横  
         for ( int i = 0;i<BoardManager.CrossCount;i++)
         {
             for ( int j = 0;j<BoardManager.CrossCount - 4 ;j++)
             {
         ...
         // 右斜
         for (int i = 0; i < BoardManager.CrossCount - 4; i++)
         {
             for (int j = 0; j < BoardManager.CrossCount - 4; j++)
             {
                 for (int k = 0; k < BoardModel.WinChessCount; k++)
         ...
         for (int i = 0; i < BoardManager.CrossCount - 4; i++)
         {
             for (int j = BoardManager.CrossCount - 1; j >= 4; j--)
             {
                 for (int k = 0; k < BoardModel.WinChessCount; k++)
                 {
                     _ptable[j - k, i + k, _icount] = true;
